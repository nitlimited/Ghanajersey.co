  ...
  return (
    <div className="min-h-screen bg-bone-white" data-testid="customer-dashboard">
      ...
      <div className="pt-32 pb-24 px-6 md:px-12 max-w-7xl mx-auto">
        {/* Welcome */}
        <div className="mb-12">
          <h1 className="font-heading text-2xl md:text-3xl tracking-widest uppercase" data-testid="dashboard-title">
            Welcome back, {user?.name?.split(' ')[0]}
          </h1>
          <p className="font-body text-muted-text mt-2">Manage your orders and account settings</p>
        </div>
        ...
        <Tabs defaultValue="orders" className="w-full">
          ...
          <TabsContent value="orders">
            {loading ? (
                  ...
                  <div key={order.order_id} className="bg-white p-6 border border-black/10" data-testid={`order-${order.order_id}`}>
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                      <div>
                        <div className="flex items-center gap-3">
                          ...
                          <span className={`font-body text-xs uppercase tracking-wider ${getStatusColor(order.payment_status)}`}>
                      ...
                      <div className="text-right">
                        ...
                        <span className={`block font-body text-xs uppercase tracking-wider mt-1 ${getStatusColor(order.order_status)}`}>
                          {order.order_status}
                        </span>
                      </div>
                    </div>
                    ...
                    {order.tracking_number && (
                      <div className="flex items-center gap-2 mt-4 pt-4 border-t border-black/10">
                        ...
                        <span className="font-body text-sm text-muted-text">Tracking: {order.tracking_number}</span>
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div className="text-center py-16 bg-white border border-black/10">
                ...
                <p className="font-body text-muted-text mb-6">Start shopping to see your orders here</p>
                <Link to="/products">
          ...
          <TabsContent value="wishlist">
            {loading ? (
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {wishlist.map((product) => (
                  <Link key={product.product_id} to={`/products/${product.product_id}`} className="group">
                    ...
                    <h3 className="font-heading text-sm tracking-wide uppercase mt-4 group-hover:text-ashanti-gold transition-colors">
                      {product.name}
                    </h3>
                    <span className="font-body text-lg">{product.currency} {product.price?.toFixed(2)}</span>
                  </Link>
                ))}
              </div>
            ) : (
              <div className="text-center py-16 bg-white border border-black/10">
                ...
                <h3 className="font-heading text-lg tracking-widest uppercase mb-2">Wishlist is empty</h3>
                <p className="font-body text-muted-text mb-6">Save your favorite items for later</p>
                <Link to="/products">
                  <Button className="bg-black text-white hover:bg-ashanti-gold hover:text-black">
                    Explore Products
                  </Button>
          ...
          <TabsContent value="account">
            <div className="bg-white p-8 border border-black/10 max-w-2xl">
              <h3 className="font-heading text-lg tracking-widest uppercase mb-6">Account Details</h3>
              
              <div className="space-y-6">
                <div className="flex items-center gap-4">
                  <div className="w-16 h-16 bg-black text-white flex items-center justify-center font-heading text-2xl">
                    {user?.name?.charAt(0)}
                  </div>
                  <div>
                    <p className="font-body font-medium text-lg">{user?.name}</p>
                    <p className="font-body text-muted-text">{user?.email}</p>
                  </div>
                </div>

                <div className="border-t border-black/10 pt-6 space-y-4">
                  <div className="flex justify-between">
                    <span className="font-body text-muted-text">Account Type</span>
                    <span className="font-body capitalize">{user?.role}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-body text-muted-text">Total Orders</span>
                    <span className="font-body">{orders.length}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-body text-muted-text">Wishlist Items</span>
                    <span className="font-body">{wishlist.length}</span>
                  </div>
                </div>
                ...
                {user?.role === "customer" && (
                  <div className="border-t border-black/10 pt-6">
                    <Link to="/auth" className="flex items-center justify-between hover:text-ashanti-gold transition-colors">
                      <span className="font-body">Become a Vendor</span>
